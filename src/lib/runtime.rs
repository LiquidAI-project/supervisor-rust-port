...
use crate::lib::stack::{CallFrame, Stack, StackAccess};
use crate::lib::table::Table;
use crate::lib::trap::{Result, Trap, TrapReason};
use crate::lib::value::{Float, LittleEndian, Value};
use crate::lib::{cast, import::DefaultImporter};
use crate::lib::wain_ast::AsValType;
use crate::lib::wain_ast::{self as ast, Module};
use crate::lib::interuption::interuption_impl::{Implementer, Interuption};
//extern crate alloc;
//use alloc::vec::Vec;
...
pub struct ModuleInstanceSerializable<'a> {
    #[serde(borrow)]
    ast: Module<'a>,
    table: Table,
    memory: Memory,
...
#[derive(Serialize, Deserialize)]
pub struct RuntimeSerialisable<'a> {
    #[serde(borrow)]
    module: ModuleInstanceSerializable<'a>,
    stack: Stack,
...
/**
 * Purpose of this struct is to record information about unfinished execution of function or block
 */
#[derive(Serialize, Deserialize, Clone)]
pub struct ControlFrame {
    pub nth_instruction: usize,
    pub prev_frame: Option<CallFrame>,
}
...
impl<'m, 's, I: Importer> Runtime<'m, 's, I> {
    ...
    pub fn instantiate(module: &'m ast::Module<'s>, importer: I, interuption_method: Arc<Implementer>) -> Result<Self> {
    ...
    pub fn get_serializable(&'_ self) -> RuntimeSerialisable<'_> {
        RuntimeSerialisable {
            module: ModuleInstanceSerializable {
    ...
    fn invoke_by_funcidx(&mut self, funcidx: u32) -> Result<bool> {
        ...
        if self.stack_trace.is_empty() {
            ...
            self.frame_stack.push(ControlFrame {
                nth_instruction: 0,
                prev_frame: Some(prev_frame),
            });
        } else {
            nth_instruction = self.stack_trace[0].nth_instruction;
            self.stack_trace.remove(0);
        }

        for insn in body.iter().skip(nth_instruction) {
            ...
            self.frame_stack[len - 1].nth_instruction += 1;
        }
        ...
        self.stack
            .pop_frame(self.frame_stack[len - 1].prev_frame.clone().unwrap(), has_result);
...
impl<'m, 's, I: Importer> Execute<'m, 's, I> for Vec<ast::Instruction> {
    fn execute(&self, runtime: &mut Runtime<'m, 's, I>) -> ExecResult {
        ...
        if runtime.stack_trace.is_empty() {
            runtime.frame_stack.push(ControlFrame {
                nth_instruction: 0,
                prev_frame: None,
            });
        } else {
            nth_instruction = runtime.stack_trace[0].nth_instruction;
            runtime.stack_trace.remove(0);
        }

        for insn in self.iter().skip(nth_instruction) {
            match insn.execute(runtime)? {
                ExecState::Continue => {
                    let len = runtime.frame_stack.len();
                    runtime.frame_stack[len - 1].nth_instruction += 1;
                }
                state => {
                    runtime.frame_stack.pop();
                    return Ok(state);
                } // Stop executing this block on return or break
            }
        }
        runtime.frame_stack.pop();
...
/**
 * This trait was created for defining how wain-exec can be asked to:
 * 1. take snapshot of a WebAssembly-module's runtime.
 * 2. continue execution of WebAssembly-module's runtime based on snapshot.
 */
pub trait Snapshot {
    fn snapshot(&self);
...
impl<'m, 's, I: Importer> Snapshot for Runtime<'m, 's, I> {
    fn snapshot(&self) {
        let buf = rmp_serde::to_vec(&self.get_serializable()).unwrap();
...
impl Snapshot for RuntimeSerialisable<'_> {
    fn snapshot(&self) {
        todo!("Not implemented!")
    }
...
impl<'m, 's, I: Importer> Execute<'m, 's, I> for ast::Instruction {
    ...
    fn execute(&self, runtime: &mut Runtime<'m, 's, I>) -> ExecResult {
        ...
        if runtime.interupt.check_interuption() {
            runtime.snapshot();
            return Err(Trap::new(TrapReason::Snapshot, self.start));
        }
        match &self.kind {
