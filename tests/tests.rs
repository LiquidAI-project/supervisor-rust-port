...
mod wasmtime_tests {
    ...
    fn test_module_loading() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for module_path in module_paths {
            ...
            match load_result {
                Ok(_) => {
                    debug!("✅ Successfully loaded: {}", module_name);
                    let _result_2 = runtime.load_module(config);
                }
                Err(e) => {
                    error!("❌ Failed to load {}: {}", module_name, e);
        ...
        debug!("Running camera test (static image size)...");
        ...
        debug!("Camera response: {:?}", camera_response);
